...
public class PinpadController : ControllerBase
{
    ...
    public async Task<IActionResult> GetBranches(string? status = null)
    {
        try
        {
            var query = _context.Branches.Include(b => b.Regional).AsQueryable();
            
            if (!string.IsNullOrEmpty(status))
            {
                query = query.Where(b => b.Class == status);
            }
    ...
    public async Task<IActionResult> GetRegionals()
    {
        try
        {
            var regionals = await _context.Regionals.Include(r => r.Branches).ToListAsync();
        ...
        try
        {
            ...
            var result = new
            {
                ...
                data = allPinpads.Select(p => new
                {
                    ...
                    regionalName = p.Branch?.Regional?.Name
                })
            };
        ...
        string? serialNumber = null)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(branch))
            {
                ...
                {
                    // If branch is not a valid integer, return error
                    return BadRequest(new { 
                        success = false, 
                        message = "Parameter 'branch' harus berupa angka yang valid.",
                        debug = new { branch, filtersApplied = new { status, branch, serialNumber } }
                    });
                }
            }
        ...
        try
        {
            ...
            var result = new
            {
                ...
                data = new
                {
                    ...
                    activePinpads,
                    inactivePinpads,
                    hasData = regionalCount > 0 || branchCount > 0 || pinpadCount > 0
                }
            };
        ...
        try
        {
            ...
            return Ok(new { 
                ...
                data = new {
                    ...
                    pinpads = pinpadCount,
                    users = userCount,
                    hasData = regionalCount > 0 || branchCount > 0 || pinpadCount > 0 || userCount > 0
                }
            });
        }
        catch (Exception ex)
    ...
    public async Task<IActionResult> SeedSampleData()
    {
        try
        {
            ...
            _context.Regionals.Add(regional);
            ...
            return Ok(new { 
                ...
                data = new { regional, branches, pinpads }
            });
        }
        catch (Exception ex)
        {
    ...
    private byte[] GeneratePinpadCsv(List<Pinpad> pinpads)
    {
        ...
        foreach (var pinpad in pinpads)
        {
            ...
            var csvRow = $"{branch?.Regional?.Name ?? ""},{parentBranch?.Code ?? branch?.Code ?? ""},{branch?.Code ?? ""},{branch?.Name ?? ""},{pinpad.PpadCreateDate:dd-MM-yyyy HH:mm:ss},{pinpad.PpadUpdateDate?.ToString("dd-MM-yyyy HH:mm:ss") ?? ""},{pinpad.PpadSn ?? ""},{pinpad.PpadTid ?? ""},{pinpad.PpadStatus ?? ""},{pinpad.PpadCreateBy ?? ""},{branch?.IpLow ?? ""},{branch?.IpHigh ?? ""},{pinpad.PpadLastActivity?.ToString("dd-MM-yyyy HH:mm:ss") ?? ""}";
        ...
        try
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                for (int i = 0; i < pinpads.Count; i++)
                {
                    ...
                    worksheet.Cells[row, 1].Value = branch?.Regional?.Name ?? "";
                    ...
                    worksheet.Cells[row, 11].Value = branch?.IpLow ?? "";
                    worksheet.Cells[row, 12].Value = branch?.IpHigh ?? "";
        ...
        using (var memoryStream = new MemoryStream())
        {
            using (var doc = new Document(PageSize.A4.Rotate(), 20, 20, 30, 30)) // Landscape dengan margin yang lebih baik
            {
                ...
                foreach (var pinpad in pinpads)
                {
                    ...
                    // Regional
                    AddStyledCell(table, branch?.Regional?.Name ?? "", dataFont, rowColor, Element.ALIGN_LEFT);
    ...
    private BranchInfo GetBranchInfo(int branchId)
    {
        var branch = _context.Branches
            .Include(b => b.Regional)
            .FirstOrDefault(b => b.Id == branchId);
        
        var parentBranch = branch?.ParentBranchId.HasValue == true 
            ? _context.Branches.FirstOrDefault(b => b.Id == branch.ParentBranchId.Value)
            : null;
            
        return new BranchInfo { Branch = branch, ParentBranch = parentBranch };
    }
        ...
        try
        {
            using (var package = new ExcelPackage())
            {
                ...
                for (int i = 0; i < pinpads.Count(); i++)
                {
                    ...
                    // Get branch and regional info using helper method
                    var branchInfo = GetBranchInfo((int)pinpad.PpadBranch);
                    var branch = branchInfo.Branch;
                    var parentBranch = branchInfo.ParentBranch;
